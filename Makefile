...
# Helm-related nonsense so this is portable
HELM_NS ?= $(shell basename $(PWD))
HELM_RELEASE ?= $(HELM_NS)-dev
...
.PHONY: kup
kup:
	@if [ `$(HELM) list | grep -c $(HELM_RELEASE)` == "0" ]; then\
	  $(HELM) install ./charts --namespace $(HELM_NS) --name $(HELM_RELEASE) --set docker_tag=$(HASH)$(DIRTY),repo_prefix=$(DOCKER_TAG_PREFIX);\
	else\
	  $(HELM) upgrade $(HELM_RELEASE) ./charts --namespace $(HELM_NS) --set docker_tag=$(HASH)$(DIRTY),repo_prefix=$(DOCKER_TAG_PREFIX);\
	fi

################
## Utilities
.PHONY: check 
check: CHECK_FOR = $(word 1, $(GO)) $(word 1, $(DOCKER)) $(word 1, $(DOCKER_COMPOSE)) $(word 1, $(DEPgB)) $(word 1, $(HELM)) $(word 1, $(KUBECTL)) $(word 1, $(NPM)) $(word 1, $(NODE)) $(word 1, $(OVERALLS))
check:
